...
export class ReviewService {
  ...
  async ratePerfume(perfumeId: string, rating: number, userId: string): Promise<void> {
    ...
    if (userRating) {
      await this.Rating.updateOne({ _id: userRating._id }, { rating, createdAt: new Date() })
    } else {
  ...
  async validateUser(accessToken: string, refreshToken: string): Promise<AuthTokenPayload> {
  ...
  async getRating(perfumeId: string, req: AuthenticatedRequest): Promise<PerfumeRatingResponse> {
    const userDetails = await this.validateUser(req.cookies['access_token'], req.cookies['refresh_token'])
    const allRatings = await this.Rating.find({
      perfume: new Types.ObjectId(perfumeId)
    })
    const averageRating = allRatings.reduce((acc, rating) => acc + rating.rating, 0)
    ...
    const userRating = allRatings.find(rating => rating.user.toString() === userDetails.id)
  ...
  async createReview(perfumeId: string, input: CreateReviewDto, userId: string): Promise<void> {
    const perfume = await this.PerfumeModel.findById(perfumeId)
    if (!perfume) {
      throw new BadRequestException('Perfume not found')
    }
